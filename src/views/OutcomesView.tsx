    ...
    <>
      ...
      {showNew && (
        <form className="panel inline-create" onSubmit={create}>
          <label><span>Outcome title</span><input value={title} onChange={(e) => setTitle(e.target.value)} required /></label>
  ...
  return (
    <div className="drawer-backdrop" role="presentation">
      <aside className="drawer" aria-label={`Outcome: ${outcome.title}`}>
        ...
        <label><span>Status</span><select value={outcome.status} onChange={(e) => runMutation(`/api/outcomes/${outcome.id}`, "PATCH", { status: e.target.value })}>{columns.map((column) => <option key={column.status} value={column.status}>{column.label}</option>)}<option value="cut">cut</option></select></label>
        ...
        <section className="drawer-section">
          ...
          {showWork && <form className="mini-form" onSubmit={addWork}><input aria-label="Work title" placeholder="Concrete deliverable" value={workTitle} onChange={(e) => setWorkTitle(e.target.value)} required /><select aria-label="Discipline" value={discipline} onChange={(e) => setDiscipline(e.target.value)}><option>code</option><option>integration</option><option>writing</option><option>game design</option><option>3d art</option><option>texturing</option><option>audio</option><option>playtesting</option></select><button className="primary-button" disabled={busy} type="submit">add</button></form>}
          <div className="drawer-work-list">{work.map((item) => <div className="drawer-work" key={item.id}><span className={`status-dot ${item.status}`} /><div><strong>{item.title}</strong><span>{item.discipline} · {item.ownerName ?? "unassigned"}</span></div><select aria-label={`status for ${item.title}`} value={item.status} onChange={(e) => runMutation(`/api/work/${item.id}`, "PATCH", { status: e.target.value })}><option>blocked</option><option>ready</option><option>doing</option><option>review</option><option>done</option></select></div>)}</div>
        </section>
        <section className="drawer-section"><p className="eyebrow">discussion</p><form className="comment-form" onSubmit={addComment}><input aria-label="Add a comment" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Keep the note attached to the outcome..." /><button className="secondary-button" disabled={!comment.trim()} type="submit">comment</button></form>{comments.map((item) => <div className="comment" key={item.id}><strong>{item.authorName}</strong><p>{item.body}</p></div>)}</section>
      </aside>
    </div>
  );
}
