...
export function PlaytestsView({ workspace, runMutation, busy }: ViewProps) {
  ...
  return (
    <>
      ...
      {showBuild && <form className="panel inline-create" onSubmit={createBuild}><label><span>Build name</span><input value={buildName} onChange={(e) => setBuildName(e.target.value)} required /></label><label><span>What became playable</span><textarea value={summary} onChange={(e) => setSummary(e.target.value)} /></label><div className="form-actions"><button className="secondary-button" onClick={() => setShowBuild(false)} type="button">cancel</button><button className="primary-button" disabled={busy} type="submit">save build</button></div></form>}
      {showPlaytest && <form className="panel inline-create three-column" onSubmit={createPlaytest}><label><span>Playtest title</span><input value={testTitle} onChange={(e) => setTestTitle(e.target.value)} required /></label><label><span>Build</span><select value={buildId} onChange={(e) => setBuildId(e.target.value)}><option value="">none</option>{workspace.builds.map((build) => <option key={build.id} value={build.id}>{build.name}</option>)}</select></label><label><span>Outcome</span><select value={outcomeId} onChange={(e) => setOutcomeId(e.target.value)}><option value="">none</option>{workspace.outcomes.map((outcome) => <option key={outcome.id} value={outcome.id}>{outcome.title}</option>)}</select></label><label className="wide"><span>Observations</span><textarea value={observations} onChange={(e) => setObservations(e.target.value)} required /></label><label><span>Decision</span><select value={decision} onChange={(e) => setDecision(e.target.value)}><option>undecided</option><option>keep</option><option>revise</option><option>cut</option></select></label><div className="form-actions"><button className="secondary-button" onClick={() => setShowPlaytest(false)} type="button">cancel</button><button className="primary-button" disabled={busy} type="submit">save playtest</button></div></form>}
