...
export function BacklogDialog({
    ...
    () =>
      pages
        .filter((page) => {
          ...
          if (chapterFilter !== null && chapterFilter !== "unplaced" && page.chapter !== chapterFilter)
            return false;
  ...
  return (
    <Drawer
      ...
      labelledBy="backlog-dialog-title"
      onClose={onClose}
    >
      <header className="dialog-header library-header">
        <div>
          ...
          <p>
            ...
            {target &&
              ` · ${pages.filter((page) => page.chapter === target.slug).length} already in ${target.name}`}
          </p>
        </div>
        <button aria-label="Close backlog" className="icon-button" onClick={onClose} type="button">
          ×
        </button>
      </header>
        ...
        {chapters.length > 0 && (
          <div aria-label="Backlog chapters" className="library-filters chapter-filters">
            ...
            <button
              ...
              className={chapterFilter === null ? "active" : ""}
            ...
            <button
              aria-pressed={chapterFilter === "unplaced"}
              className={chapterFilter === "unplaced" ? "active" : ""}
              onClick={() => setChapterFilter(chapterFilter === "unplaced" ? null : "unplaced")}
              type="button"
            >
              no chapter
            </button>
      ...
      <div className="library-results" aria-live="polite">
        ...
        {visiblePages.length === 0 && (
          <div className="library-empty">
            ...
            <span>
              {pages.length === 0
                ? "Capture work above whenever something earns a place here."
...
function isBlocked(page: Page, allPages: Page[]): boolean {
  return page.blockedBy.some((id) =>
    allPages.some((candidate) => candidate.id === id && candidate.status !== "done"),
  );
}
