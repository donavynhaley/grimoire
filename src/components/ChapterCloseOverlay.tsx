...
type Props = {
  ...
  /** Creates a chapter and opens it, so the project is never left without a current one. */
  onCreateChapter: (name: string) => Promise<boolean>;
};
...
export function ChapterCloseOverlay({
  ...
  const submitNew = async (event: FormEvent) => {
    ...
    if (!trimmed) return;
    if (await onCreateChapter(trimmed)) onDismiss();
  };
          ...
          <>
            ...
            {naming && (
              <form className="chapter-close-new" onSubmit={(event) => void submitNew(event)}>
                ...
                <button disabled={busy} onClick={() => setNaming(false)} type="button">
                  back
                </button>
              </form>
            )}
