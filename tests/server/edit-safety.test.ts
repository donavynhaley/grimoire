...
function cardPath(server: Server, cardId: string): string {
  return join(server.cardsDirectory, "wizard-simulator", "cards", `${cardId}.md`);
}

async function createCard(server: Server, title: string, description: string) {
...
describe("concurrent edit safety", () => {
  it("refuses a write against notes that already changed, and leaves them intact", async () => {
    ...
    const card = await createCard(server, "Potion workbench", "The bench holds three reagents.");
    ...
    const stale = await server.request<{ error: string; conflict: boolean; field: string; current: Card }>(
      `/api/cards/${card.id}`,
      {
        method: "PATCH",
        body: JSON.stringify({
  ...
  it("lets an untouched field through while a different field is contested", async () => {
    const server = await startTestServer();
    await bootstrap(server);
    const card = await createCard(server, "Cauldron bubbles", "Needs to read over the UI.");
    ...
    expect(renamed.response.status).toBe(200);
    expect(renamed.body.card.description).toBe("Second pass is on the branch.");
  });

  it("leaves column moves and reordering as last writer wins", async () => {
    const server = await startTestServer();
    await bootstrap(server);
    const card = await createCard(server, "Rain particles", "Low priority.");
    ...
    const moved = await server.request<{ card: Card }>(`/api/cards/${card.id}`, {
      method: "PATCH",
      body: JSON.stringify({ status: "ready", position: 0 }),
    });

    expect(moved.response.status).toBe(200);
    expect(moved.body.card.status).toBe("ready");
    expect(moved.body.card.description).toBe("Raised after the playtest.");
  });
  ...
  it("accepts a rewrite that expects the value actually stored", async () => {
    const server = await startTestServer();
    await bootstrap(server);
    const card = await createCard(server, "Save slot corruption", "Reproduces on quick quit.");

    const saved = await server.request<{ card: Card }>(`/api/cards/${card.id}`, {
      method: "PATCH",
      body: JSON.stringify({
    ...
    expect(saved.body.card.description).toBe("Reproduces within 200ms of the autosave toast.");
  });
});
