...
describe("while you were away - quiet signals", () => {
  ...
  it("draws the unread line where the last visit ended", async () => {
    const away: AwayState = { since: 5, latest: 8, total: 2, events: [awayEvent({ sequence: 7 }), awayEvent({ sequence: 8 })] };
    const activity = {
      events: [
  ...
  it("shows the digest, marks changed pages, and dismiss clears everything at once", async () => {
    ...
    const away: AwayState = {
      ...
      total: 2,
      events: [
        awayEvent({ sequence: 6, action: "updated", entityId: changed.id, entityTitle: changed.title, changes: [{ field: "notes", from: null, to: "sketch" }] }),
        awayEvent({ sequence: 7, action: "moved", entityId: changed.id, entityTitle: changed.title, changes: [{ field: "column", from: "Up Next", to: "In progress" }] }),
      ],
    };
  ...
  it("clears a page's dot as soon as the page is opened", async () => {
    ...
    const away: AwayState = {
      ...
      latest: 7,
      total: 1,
      events: [awayEvent({ sequence: 6, action: "updated", entityId: changed.id, entityTitle: changed.title, changes: [{ field: "notes", from: null, to: "x" }] })],
    };
    stubFetch(authenticatedFetch(board), { away });
  ...
  it("keeps the digest to five lines and expands in place", async () => {
    ...
    events.push(awayEvent({ sequence: 30, entityType: "idea", entityId: "00000000-0000-4000-8000-000000000040", entityTitle: "Rune spells", action: "moved", changes: [{ field: "list", from: "Idea inbox", to: "Shortlist" }] }));
    events.push(awayEvent({ sequence: 31, entityType: "idea", entityId: "00000000-0000-4000-8000-000000000041", entityTitle: "Tower garden", action: "created" }));
    events.push(awayEvent({ sequence: 32, entityType: "member", entityId: "00000000-0000-4000-8000-000000000013", entityTitle: "Alex", action: "joined", actorName: "Alex" }));
  ...
  it("advances the seen cursor on load only while the tab is visible", async () => {
    ...
    await waitFor(() => expect(calls.some((call) => call.url.startsWith("/api/seen") && call.method === "POST")).toBe(true));
  });
});
