...
**Single sign-on is included, and it is free.** Set three variables — your provider's issuer URL, a client id, and a client secret — and a button appears beside the password form. Any OpenID Connect provider works, because Grimoire talks the protocol rather than a list of vendors: Authentik, Keycloak, Authelia, Pocket ID, Entra, Google. Register `https://your-grimoire/api/auth/oidc/callback` with the provider and the rest is [.env.example](.env.example).
...
An account is found by email. Somebody who has been signing in with a password keeps their history, their memberships, and their name the moment single sign-on is turned on; the provider signs them in, it does not replace them. Somebody with no account gets one only if they followed an invitation, because a provider vouching for a person is not by itself a reason to put them on a board. An operator whose provider is their own team's can say `GRIMOIRE_OIDC_SIGNUP=open` and skip the invitation, which is the right setting for exactly that case and the wrong one for a shared or public provider.
